            ...
            child: Column(
              children: [
                Row(
                  ...
                  children: [
                    SizedBox(width: calendarElementSize, height: calendarElementSize, child: const Text("일", textAlign: TextAlign.center) ),
                    SizedBox(width: calendarElementSize, height: calendarElementSize, child: const Text("월", textAlign: TextAlign.center) ),
                    SizedBox(width: calendarElementSize, height: calendarElementSize, child: const Text("화", textAlign: TextAlign.center) ),
                    SizedBox(width: calendarElementSize, height: calendarElementSize, child: const Text("수", textAlign: TextAlign.center) ),
                    SizedBox(width: calendarElementSize, height: calendarElementSize, child: const Text("목", textAlign: TextAlign.center) ),
                    SizedBox(width: calendarElementSize, height: calendarElementSize, child: const Text("금", textAlign: TextAlign.center) ),
                    SizedBox(width: calendarElementSize, height: calendarElementSize, child: const Text("토", textAlign: TextAlign.center) ),
                  ],
                ),
                SizedBox(
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        if (data.isEmpty) {
          ...
          List<List<DateTime>> datePartition = partitionByWeek(firstDate, lastDate);

          List<DateTime> firstRow = datePartition.first;
          List<DateTime> lastRow = datePartition.last;
          List<bool> doneListFirstRow = firstRow.map((date) => data.keys.contains(date)).toList();

          List<Widget> children = [
            SizedBox(
              width: elementSize * 7,
              height: elementSize,
              child: RowContent(dates: firstRow, doneList: doneListFirstRow, isFirst: true),
            ),
          ];

          if (datePartition.length > 1) {
            List<List<DateTime>> middleRows = datePartition.sublist(1, datePartition.length - 1);

            for (var row in middleRows) {
              List<bool> doneList = row.map((date) => data.keys.contains(date)).toList();
            ...
            List<bool> doneListLastRow = lastRow.map((date) => data.keys.contains(date)).toList();

            children.add(
              SizedBox(
                width: elementSize * 7,
                height: elementSize,
                child: RowContent(dates: lastRow, doneList: doneListLastRow, isLast: true),
              ),
            );
          }
...
class RowContent extends StatelessWidget {
  ...
  const RowContent({super.key, required this.dates, required this.doneList, this.isFirst = false, this.isLast = false});

  @override
  Widget build(BuildContext context) {
    bool containsStartOfMonth = dates.any((date) => date.day == 1);
    bool containsEndOfMonth = dates.any((date) => date.day == DateTime(date.year, date.month + 1, 0).day);
      ...
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        return Opacity(
          ...
          child: Stack(
            children: <Widget>[
              Row(
                mainAxisAlignment: isFirst ? MainAxisAlignment.end : MainAxisAlignment.start,
                children: dates.asMap().entries.map((entry) {
                  int index = entry.key;
                  DateTime date = entry.value;
                  bool isDone = doneList[index];
                  List<Color> colors = isDone ? [Colors.orange.withOpacity(0.7), Colors.red.withOpacity(0.7)] : [Colors.grey.withOpacity(0.7), Colors.grey.withOpacity(0.7)];
